...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const monthlyRate = dayRate(ratePerHour) * daysInMonth;
  const discountedMonthlyRate = monthlyRate * (1 - discount);
  const fullMonthsCost = fullMonths * discountedMonthlyRate;

  // Calculate the cost for the remaining days without discount
  const remainingDaysCost = remainingDays * ratePerHour * hoursPerDay;

  // Sum the costs and round up to the nearest whole number
